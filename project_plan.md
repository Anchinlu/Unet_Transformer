...
**Mục tiêu:** Phân tích chuyên sâu (Ablation Study) để chứng minh sự vượt trội của TransUNet.
*   **Thực hiện:**
    *   [ ] Trực quan hóa 4 cột so sánh: Ảnh MRI Gốc | Mask Thực tế | Dự đoán của U-Net | Dự đoán của TransUNet.
    *   [ ] Đánh giá định lượng (Quantitative): Lập bảng so sánh Dice Score, IoU giữa 2 mô hình để thấy rõ phần trăm cải thiện.

## Giai đoạn 7: Xây dựng Giao diện Ứng dụng Web (Web UI - Tăng điểm Đồ án)
**Mục tiêu:** Biến đoạn code khô khan thành một ứng dụng y tế có thể tương tác trực tiếp.
*   **Thực hiện:**
    *   [ ] Sử dụng thư viện `Streamlit` hoặc `Gradio`.
    *   [ ] Xây dựng giao diện cho phép Upload ảnh MRI sọ não.
    *   [ ] Gọi mô hình `transunet_best_model.pth` chạy ngầm và in kết quả khoanh vùng đỏ trực tiếp lên màn hình trang Web.
